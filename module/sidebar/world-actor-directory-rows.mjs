...
export function resolveWorldActorFromDirectoryId(id) {
  if (!id) return null;
...
/** Tag world actor directory rows with `data-cts-actor-type` for filters / diagnostics. */
export function tagWorldActorDirectoryRows(root) {
  ...
  for (const el of nodes) {
    ...
    const doc = resolveWorldActorFromDirectoryId(id ?? undefined);
    if (doc?.type) el.dataset.ctsActorType = doc.type;
    else el.dataset.ctsActorType = "__unknown";
  }
}
